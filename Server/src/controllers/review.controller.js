    ...
    static async remove(req, res) {
        try {
            ...
            if (review.userId.toString() !== req?.user?._id.toString()) {
                res.status(403).json({ message: "Access denied, you are unauthorized person", status: "failed" })
            }
            const { productId } = review;
            await reviewModel.findByIdAndDelete(id);
            await updateProductRating(productId);
            res.status(200).json({ message: "Rating remove successfully", status: "success" })
        } catch (error) {
            console.log(error);
            res.status(500).json({ message: "Interval Server Error", status: "failed" })
        }
